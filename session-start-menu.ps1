...
$o.Add("")
$o.Add("使用者選定專案 X 後，我要依序做（第 1 步最優先，先鎖定書籤再讀檔）：")
$o.Add("1. 先把 $changes\X 這個完整路徑寫進 $pointer（UTF-8 無 BOM）。這步要最先做，確保書籤先鎖定。")
$o.Add("2. 讀 $changes\X\tasks.md（任務清單）。")
$o.Add("3. 讀 $changes\X\STATE.md（目前現況快照）。不存在的檔就略過。")
$o.Add("4. 用白話回報該專案目前進度與待辦，再接著做。")
$o.Add("（memory.md 不主動載入。使用者查詢具體數值時，先用 Grep 掃描 memory.md 找相關段落，再用 Read 只讀那幾行。）")
[Console]::Out.Write(($o -join "`n"))
exit 0
